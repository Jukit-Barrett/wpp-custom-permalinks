...
Simple Database 是使用PHP开发的数据库操作工具。
...
```sh
$ composer require mrzkit/simple-database
```
...
在 `test/Unit` 目录中，编写了详细的测试用例。具体可参考 `Mrzkit\SimpleDatabaseTest\Unit\SimpleDatabaseTest`类 和`Mrzkit\SimpleDatabaseTest\Unit\SimpleDatabaseTest`类。
...
其中，`$table` 属性是必填项，如果未声明 `$tablePrefix`，则采用配置文件`DatabaseConfig::getConnectionConfig()['connections']['prefix']`字段作为前缀。
